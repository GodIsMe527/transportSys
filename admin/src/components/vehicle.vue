<template>
    <div class="vehicle">
        <header>
            <!-- Form -->
            <el-button class="addIcon" type="success" @click="showLayer(0)">新增车辆</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            delUser(row) {
                ...
                api.deleteVehicle(data).then(res => {
                    if (res.data.code == 0) {
                        this.clearUserDate();
                        this.getList();
                        this.$message({
            ...
            showLayer(type, row) {
                ...
                if (type == 0) {
                    this.form.name = "";
                    this.layerTitle = "新增车辆";
                } else {
                    this.form.name = this.operateUser.no;
                    this.form.type = this.operateUser.type;
                    this.layerTitle = "编辑车辆";
                }
            },
            //保存新增/修改车辆
            newPoint() {
                if (this.form.name) {
                    ...
                    if (this.layerTitle === "新增车辆") {
                        api.newVehicle(data).then(res => {
                            if (res.data.code == 0) {
                                this.$message({
                                    message: '添加成功',
                                    type: 'success'
